# Usage Instructions — modules/contacts/models.json

| Field | Value |
|---|---|
| Document name | `modules/contacts/models.json` |
...
Verifying that a business object, field, relation or status value exists in `contacts` in Odoo 19.0; grounding functional claims; data-mapping and migration design.
...
> Source-derived data model of the Odoo 19.0 `contacts` module (Community): models defined/extended, key fields with relations and status values, built-in capabilities from mixins. High authority that these structures exist; contains no code and does not prove runtime behaviour.
